<template>
  <div class="app-container">
    ...
    >
      ...
      >
        <template slot-scope="scope">
          <span>{{ parseGameId(scope.row) }}</span>
        </template>
      </el-table-column>
        ...
        <el-row>
          <el-col :span="24">
            <el-form-item label="角色类型" prop="menuType">
              <el-radio-group
                :disabled="'修改菜单' == title || roleTyeDisabled"
          ...
          <el-col v-if="form.roleType === 2">
            <el-form-item label="是否共享" prop="menuType">
              <el-radio-group
                :disabled="'修改菜单' == title || roleTyeDisabled"
                v-model="form.isCommon"
                @change="isCommonChange"
              >
                <el-radio
                  v-for="item in isCommonOptions"
                  :key="item.dictCode"
                  :label="item.dictValue * 1"
                  >{{ item.dictLabel }}</el-radio
                >
          ...
          <el-col v-if="form.roleType === 2 && !form.isCommon" :span="24">
            <el-form-item label="游戏名称" prop="menuType">
              <el-select
                :disabled="'修改菜单' == title || gameIdDisabled"
                v-model="form.gameId"
                placeholder="请选择"
                @change="gameIdChange"
              >
...
export default {
  ...
  created() {
    ...
    this.getDicts("isCommon").then((response) => {
  ...
  methods: {
    ...
    isCommonChange(val) {
      ...
      this.menuOptionsQueryParams.isCommon = val;
      this.form.parentId = 0;
    },
    ...
    handleUpdate(row) {
      ...
      getMenu(row.menuId).then((response) => {
        ...
        this.menuOptionsQueryParams.isCommon = response.data.isCommon;
        this.getTreeselect();
        this.open = true;
        this.title = "修改菜单";
      });
